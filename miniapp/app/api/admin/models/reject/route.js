...
export async function POST(request) {
  ...
  await query(
    `UPDATE model_profiles
     SET verification_status = 'rejected', approved_at = NOW(), approved_by = $1
     WHERE user_id = $2`,
    [adminUserId, userId]
  );
  await query(
  ...
  await query(
    `INSERT INTO admin_actions (admin_id, action_type, target_user_id, target_type, target_id, details, created_at)
     VALUES ($1, 'reject_model', $2, 'model_profile', $3, $4, NOW())`,
    [adminUserId, userId, profileId, JSON.stringify({ status: "rejected" })]
  );
